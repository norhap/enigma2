...
class EPGList(GUIComponent):
	def __init__(self, type=EPG_TYPE_SINGLE, selChangedCB=None, timer=None):
		...
		self.catchUpIcon = LoadPixmap(cached=True, path=resolveFilename(SCOPE_GUISKIN, "icons/ico_catchup.png"))
		main_icons = (
			"epgclock",
			"zapclock",
			"zaprecclock",
	...
	def buildSingleEntry(self, service, eventId, beginTime, duration, EventName):
		...
		if clock_types:
			for i in range(len(clock_types)):
				...
				res.append((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND, r3.x + i * self.space, r3.y + self.dy, self.iconSize, self.iconSize, clockIcon))
	...
	def fillSingleEPG(self, service, sorting=0, filtering=0):
		...
		t = int(time())
		epg_time = t - config.epg.periodoftime.getValue() * 60
