...
from Tools.Directories import resolveFilename, fileReadLine, fileReadLines, fileWriteLine, SCOPE_KEYMAPS, SCOPE_GUISKIN, fileReadXML
...
class InputDevices:
	def __init__(self):
		...
		for device in sorted(listdir("/dev/input/")):
			if isdir("/dev/input/%s" % device):
				continue
			if device not in ("mouse0", "mice"):
				try:
					buffer = b"\0" * 512
					self.fd = open("/dev/input/%s" % device, O_RDWR | O_NONBLOCK)
					self.name = ioctl(self.fd, self.EVIOCGNAME(256), buffer).decode("UTF-8", "ignore")
	...
	def setDeviceDefaults(self, device):
		...
		fd = open("/dev/input/%s" % device, O_RDWR)
		write(fd, eventRepeat)
		write(fd, eventDelay)
		close(fd)
