...
class FrontendInfo(Converter):
	...
	@cached
	def getBool(self):
		assert self.type in (self.LOCK, self.BER, self.SNR, self.SNRdB, self.AGC, self.STRING, self.USE_TUNERS_STRING, self.REC_TUNER), "the boolean output of FrontendInfo can only be used for lock, BER, SNR, SNRdB, AGC, STRING, or  USE_TUNERS_STRING"
		swapsnr = config.usage.swap_snr_on_osd.value
