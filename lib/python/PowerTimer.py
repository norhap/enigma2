...
class PowerTimer(Timer):
	...
	def getNextPowerManagerTime(self):
		...
		fakeTime = time() + 300
		if hasattr(self, "timeshift") and config.usage.timeshift.isRecording.value:
		...
		if next_state == self.StatePrepared:
			...
			if self.timerType == TIMERTYPE.WAKEUP:
				...
				if not Screens.Standby.inStandby:  # Not already in standby.
					AddNotificationWithUniqueIDCallback(self.sendStandbyNotification, "PT_StateChange", MessageBox, _("A finished powertimer wants to set your receiver to standby.\nDo that now?"), timeout=180)
				return True
