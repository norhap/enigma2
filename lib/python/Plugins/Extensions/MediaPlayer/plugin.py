...
from Tools.Directories import fileExists, resolveFilename, SCOPE_CONFIG, SCOPE_PLAYLIST, SCOPE_GUISKIN, fileContains
...
class MediaPlayer(Screen, InfoBarBase, InfoBarScreenSaver, InfoBarSeek, InfoBarAudioSelection, InfoBarCueSheetSupport, InfoBarNotifications, InfoBarSubtitleSupport, HelpableScreen, InfoBarAspectSelection):
	...
	def __init__(self, session, args=None):
		...
		if defaultDir == "":
			defaultDir = None
		if not fileContains("/etc/enigma2/settings", "defaultDir=None"):
			config.mediaplayer.defaultDir.save()
		self.filelist = FileList(defaultDir, matchingPattern=r"(?i)^.*\.(dts|mp3|wav|wave|wv|oga|ogg|flac|m4a|mp2|m2a|wma|ac3|mka|aac|ape|alac|mpg|vob|m4v|mkv|avi|divx|dat|flv|mp4|mov|wmv|asf|3gp|3g2|mpeg|mpe|rm|rmvb|ogm|ogv|m2ts|mts|ts|stream|m3u|e2pls|pls|amr|au|mid|pva|wtv)", useServiceRef=True, additionalExtensions="4098:m3u 4098:e2pls 4098:pls")
	...
	def save_playlist2(self, name):
		if self.playlist.list:
			if name is not None:
				name = name.strip()
				if name == "":
					name = time.strftime("%y%m%d_%H%M%S.e2pls")
