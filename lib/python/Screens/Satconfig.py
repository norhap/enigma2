...
from Components.config import getConfigListEntry, config, ConfigNothing, configfile, ConfigBoolean, ConfigSelection
...
class NimSetup(Setup, ServiceStopScreen):
	def __init__(self, session, slotid):
		...
		Setup.__init__(self, session, None,
			yellow_button={'function': self.key_yellow, 'helptext': _("Toggle Configuration Mode or Auto DiSEqC")},
	...
	def autoDiseqcRun(self, autodiseqc_ports):
		self.stopService()
		...
		self.session.openWithCallback(self.autoDiseqcCallback, AutoDiseqc, self.slotid, autodiseqc_ports, order)
	...
	def saveAll(self, reopen=False):
		...
		if self.isChanged():
			...
			from Components.RTLSDR import updateDABBoxInfo  # noqa: E402
			updateDABBoxInfo()
