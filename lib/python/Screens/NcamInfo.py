...
class NcamInfo:
	...
	def getUserData(self):
		...
		# Assume that ncam webif is NOT blocking localhost, IPv6 is also configured if it is compiled in,
				...
				with open(conf, 'r') as data:
					for i in data:
						if "httpuser" in i.lower():
							...
							# Once we encounter a httpallowed statement, we have to assume ncam webif is blocking us ...
							blocked = True
...
class NcamInfoConfigScreen(Screen, ConfigListScreen):
	...
	def createSetup(self):
		self.oscamconfig = []
		self.oscamconfig.append(getConfigListEntry(_("Read Userdata from ncam.conf"), config.ncaminfo.userdatafromconf))
