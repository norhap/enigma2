...
#			if config.plugins.ncaminfo.usehostname.value:
#				self.ip = socket.gethostbyname(config.plugins.ncaminfo.hostname.value)
#			else:
			self.ip = ".".join("%d" % d for d in config.ncaminfo.ip.value)
			self.port = config.ncaminfo.port.value
			self.username = config.ncaminfo.username.value
			self.password = config.ncaminfo.password.value
		if part is None:
			self.url = "http://%s:%s/ncamapi.html?part=status" % ( self.ip, self.port )
		else:
			self.url = "http://%s:%s/ncamapi.html?part=%s" % (self.ip, self.port, part )
		if part is not None and reader is not None:
			self.url = "http://%s:%s/ncamapi.html?part=%s&label=%s" % ( self.ip, self.port, part, reader )
...
class NcamInfoConfigScreen(Screen, ConfigListScreen):
	...
	def createSetup(self):
		self.oscamconfig = []
		self.oscamconfig.append(getConfigListEntry(_("Read Userdata from oscam.conf/ncam.conf"), config.ncaminfo.userdatafromconf))
		if not config.ncaminfo.userdatafromconf.value:
			self.oscamconfig.append(getConfigListEntry(_("Username (httpuser)"), config.ncaminfo.username))
			self.oscamconfig.append(getConfigListEntry(_("Password (httpwd)"), config.ncaminfo.password))
			self.oscamconfig.append(getConfigListEntry(_("IP address"), config.ncaminfo.ip))
			self.oscamconfig.append(getConfigListEntry(_("Port"), config.ncaminfo.port))
		self.oscamconfig.append(getConfigListEntry(_("Automatically update Client/Server View?"), config.ncaminfo.autoupdate))
		if config.ncaminfo.autoupdate.value:
			self.oscamconfig.append(getConfigListEntry(_("Update interval (in seconds)"), config.ncaminfo.intervall))
