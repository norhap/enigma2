...
class ServiceScan(Screen):
	...
	def __init__(self, session, scanList):
		...
		self["actions"] = ActionMap(["SetupActions", "MenuActions", "NavigationActions"], {
			"ok": self.keySave,
			"save": self.keySave,
			"cancel": self.keyCancel,
			"menu": self.keyCloseRecursive,
			"top": self["servicelist"].goTop,
			"pageUp": self["servicelist"].goPageUp,
			"up": self["servicelist"].goLineUp,
			"down": self["servicelist"].goLineDown,
			"pageDown": self["servicelist"].goPageDown,
			"bottom": self["servicelist"].goBottom
		}, -2)
