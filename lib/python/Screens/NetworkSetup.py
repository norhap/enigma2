...
class NetworkAdapterSelection(Screen, HelpableScreen):
	...
	def buildInterfaceList(self, iface, name, default, active):
		...
		activepng = LoadPixmap(cached=True, path=resolveFilename(SCOPE_GUISKIN, "icons/lock_on.png")) if active else LoadPixmap(cached=True, path=resolveFilename(SCOPE_GUISKIN, "icons/lock_error.png"))
...
class DNSSettings(Setup, HelpableScreen):
	...
	def updateStatus(self):
		if config.usage.dns.value not in ("dhcp-router", "custom"):
			self["key_yellow"].setText("")
...
class AdapterSetup(ConfigListScreen, HelpableScreen, Screen):
	...
	def applyConfig(self, ret=False):
		...
		if not staticip and config.usage.dns.value == "staticip":
			config.usage.dns.value = "dhcp-router"
			config.usage.dns.save()
		elif staticip and config.usage.dns.value == "dhcp-router":
			config.usage.dns.value = "staticip"
			config.usage.dns.save()
		if ret:
			self.applyConfigRef = None
