...
def InitSkins():
	...
	skinTemplatesFileName = resolveFilename(SCOPE_SKINS, join(dirname(str(currentPrimarySkin)), "skinTemplates.xml"))
	...
	for name in names:  # Try all skins, first existing one has priority.
		...
		if myScreen is not None:
			...
			if screen.mandatoryWidgets == [] or all(item in widgets for item in screen.mandatoryWidgets):
				...
				print(f"[Skin] Warning: Skin screen '{name}' rejected as it does not offer all the mandatory widgets '{joincomma.join(screen.mandatoryWidgets)}'!")
				myScreen = None
	else:
